...
public class M60Model implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrix, MultiBufferSource renderingbuffer, int light, int overlay){
        ...
        if (Gun.getScope(stack) == null) {
            RenderUtil.renderModel(SpecialModels.M60_IRONSIGHTS.getModel(), stack, matrix, renderingbuffer, light, overlay);
        } else {
        ...
        RenderUtil.renderModel(SpecialModels.M60_CHARGING_HANDLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        matrix.popPose();
    }
