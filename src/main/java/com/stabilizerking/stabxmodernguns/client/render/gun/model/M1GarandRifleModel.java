...
public class M1GarandRifleModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrix, MultiBufferSource renderingbuffer, int light, int overlay){
        ...
        RenderUtil.renderModel(SpecialModels.M1_GRANDE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        ...
        if (Gun.getScope(stack) == null) {
            RenderUtil.renderModel(SpecialModels.M1_GRANDE_IRONSIGHTS.getModel(), stack, matrix, renderingbuffer, light, overlay);
        } else {
            RenderUtil.renderModel(SpecialModels.M1_GRANDE_IRONSIGHTS_MOUNT.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        //this will render the useless charging handle
        RenderUtil.renderModel(SpecialModels.M1_GARAND_CHAGING_HANDLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        matrix.popPose();
