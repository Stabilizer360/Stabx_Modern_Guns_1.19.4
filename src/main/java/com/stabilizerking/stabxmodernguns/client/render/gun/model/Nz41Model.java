...
public class Nz41Model implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrix, MultiBufferSource renderingbuffer, int light, int overlay){
        RenderUtil.renderModel(SpecialModels.NZ41.getModel(), stack, matrix, renderingbuffer, light, overlay);
        ...
        if (Gun.getScope(stack) == null) {
            RenderUtil.renderModel(SpecialModels.NZ41_IRONSIGHTS.getModel(), stack, matrix, renderingbuffer, light, overlay);
        } else {
            RenderUtil.renderModel(SpecialModels.NZ41_IRONSIGHTS_MOUNT.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        RenderUtil.renderModel(SpecialModels.NZ41_CHARGING_HANDLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        //Always pop
        matrix.popPose();
