...
public class M1GarandRemakeModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrix, MultiBufferSource renderingbuffer, int light, int overlay){
        ...
        RenderUtil.renderModel(SpecialModels.M1_GARAND_REMAKE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        ...
        if (Gun.getScope(stack) == null) {
            RenderUtil.renderModel(SpecialModels.M1_GARAND_REMAKE_IRONSIGHTS.getModel(), stack, matrix, renderingbuffer, light, overlay);
        } else {
            RenderUtil.renderModel(SpecialModels.M1_GARAND_REMAKE_IRONSIGHTS_MOUNT.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        RenderUtil.renderModel(SpecialModels.M1_GARAND_REMAKE_CHARGING_HANDLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        matrix.popPose();
