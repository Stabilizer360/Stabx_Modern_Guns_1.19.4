...
public class Hcar762model implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext display, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrix, MultiBufferSource renderingbuffer, int light, int overlay){
        RenderUtil.renderModel(SpecialModels.HCAR762.getModel(), stack, matrix, renderingbuffer, light, overlay);

        //Checks if therir is no scope putted
        if (Gun.getScope(stack) == null) {
            //firsy renders the default ironsight
            RenderUtil.renderModel(SpecialModels.HCAR762_IRONSIGHTS.getModel(), stack, matrix, renderingbuffer, light, overlay);
        } else {
            //when scope is putted it will render the 2nd ironsights model
            RenderUtil.renderModel(SpecialModels.HCAR762_IRONSIGHTS_MOUNT.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        if (Gun.getAttachment(IAttachment.Type.BARREL, stack).getItem() == ModItems.SILENCER.get()) {
            RenderUtil.renderModel(SpecialModels.HCAR762_SUPPRESSOR.getModel(), stack, matrix, renderingbuffer, light, overlay);

        } else if (Gun.getAttachment(IAttachment.Type.BARREL, stack).getItem() == ModItemRegisteration.ADVANCED_SUPPRESSOR.get()) {
            RenderUtil.renderModel(SpecialModels.HCAR762_ADVANCED_SUPPRESSOR.getModel(), stack, matrix, renderingbuffer, light, overlay);

        } else if (Gun.getAttachment(IAttachment.Type.BARREL, stack).getItem() == ModItemRegisteration.ADVANCED_MUZZLE_BRAKE.get()) {
            RenderUtil.renderModel(SpecialModels.HCAR762_ADVANCED_MUZZLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        //In case of any attachment is not putted if I Nine Zero Gun mod releases his new update i will decently add the compatibility
            ...
            RenderUtil.renderModel(SpecialModels.HCAR762_DEFAULT_MUZZLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItemRegisteration.ANGLED_FOREGRIP.get()) {
            RenderUtil.renderModel(SpecialModels.HCAR762_ANGLED_GRIP.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItemRegisteration.SKELETON_FOREGRIP.get()) {
            RenderUtil.renderModel(SpecialModels.HCAR762_SKELETON_GRIP.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItemRegisteration.VERTICAL_FOREGRIP.get()) {
            RenderUtil.renderModel(SpecialModels.HCAR762_VERTICAL_GRIP.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        RenderUtil.renderModel(SpecialModels.HCAR762_CHARGING_HANDLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        //Always pop
        matrix.popPose();
