...
public class Aks74uxGunModel implements IOverrideModel {
    ...
    public void render(float partialTicks,  ItemDisplayContext display, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrix, MultiBufferSource renderingbuffer, int light, int overlay){
        ...
        RenderUtil.renderModel(SpecialModels.AKS_74_UX.getModel(), stack, matrix, renderingbuffer, light, overlay);
        ...
        if (Gun.getScope(stack) == null) {
            RenderUtil.renderModel(SpecialModels.AKS74UX_IRONSIGHTS.getModel(), stack, matrix, renderingbuffer, light, overlay);
        } else {
            RenderUtil.renderModel(SpecialModels.AKS74UX_IRONSIGHTS_MOUNT.getModel(), stack, matrix, renderingbuffer, light, overlay);
        }
        ...
        RenderUtil.renderModel(SpecialModels.AKS74UX_CHARGING_HANDLE.getModel(), stack, matrix, renderingbuffer, light, overlay);
        //Always pop
        matrix.popPose();
